...
using se3 = gpw::blat::vec<6>;
using so3 = gpw::blat::vec<3>;
...
std::string to_string (const SE3&, output_fmt fmt = output_fmt::nml);
...
template <typename T>
bool similar (
    const T&, const T&, const T tol = std::numeric_limits<T>::epsilon()
);
 ...
 */
SO3 relative (const SO3&, const SO3&);
 ...
 */
SE3 relative (const SE3&, const SE3&);
...
bool operator== (const SE3&, const SE3&);
