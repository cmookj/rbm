...
SO3::SO3() : mat3{{{1., 0., 0.}, {0., 1., 0.}, {0., 0., 1.}}} {}
 ...
 */
SO3::SO3(const mat3 &M) : mat3{{{1., 0., 0.}, {0., 1., 0.}, {0., 0., 1.}}} {
  double dtm = det(M);
  if (dtm != 0) {
    auto SVD = svd(M);
...
/**
 ...
 given as 6 scalars.
 */
SE3::SE3(const double &w1, const double &w2, const double &w3, const double &v1,
...
void SE3::_expm(vec3 w, vec3 v) {
  ...
  if (theta != 0.) {
    ...
    position = vec3{{(theta - b * (w(2) * w(2) + w(3) * w(3))) * v(1) +
                         (b * w(1) * w(2) - a * w(3)) * v(2) +
                         (b * w(3) * w(1) + a * w(2)) * v(3),
                     (b * w(1) * w(2) + a * w(3)) * v(1) +
...
template <typename T> bool gpw::geometry::similar(const T &a, const T &b) {
...
mat3 gpw::geometry::skew(const double &v1, const double &v2, const double &v3) {
...
vector<vector<vec3>> gpw::geometry::interpolate(const vector<double> &t,
                                                const vector<SO3> &R,
                                                const vec3 &omega,
                                                const vec3 &alpha) {
  size_t n = t.size() - 1;
...
SO3 gpw::geometry::rotation_interpolated(const double t,
  ...
  return R[i - 1] * expm(abc[i][0] * tau * tau * tau + abc[i][1] * tau * tau +
                         abc[i][2] * tau);
}
